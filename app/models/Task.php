...
class Task
{
    public static function getAll(int $userId)
    {
        ...
        $stmt = $db->prepare("SELECT * FROM tasks WHERE user_id = ? ORDER BY (CASE WHEN status = 'completed' THEN 1 ELSE 0 END) ASC, created_at DESC");
        $stmt->execute([$userId]);
    ...
    public static function create(int $userId, int $planId, string $description, string $skillName = null, string $status = 'pending')
    {
        ...
        $stmt = $db->prepare("INSERT INTO tasks (user_id, plan_id, title, description, skill_name, status) VALUES (?, ?, ?, ?, ?, ?)");
        return $stmt->execute([$userId, $planId, $title, $description, $skillName, $status]);
    }

    public static function markAsDone(int $taskId, int $userId)
    {
        $db = Database::getInstance()->getConnection();
        $stmt = $db->prepare("UPDATE tasks SET status = 'completed' WHERE id = ? AND user_id = ?");
